...
- **Grounded regulation answers (hybrid RAG)** — answers about academic rules, graduation, and scholarships are returned with their source documents through lexical + vector-embedding **RRF fusion**. Production has the pgvector extension, HNSW index, and vector data ready, while the live serving path stays on simpler exact in-memory cosine because the corpus is only 217 chunks. Resumable incremental persistence keeps the corpus complete without repeatedly spending embedding quota.
